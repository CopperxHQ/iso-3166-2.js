...
import { getFlagEmoji } from './country-data';
...
export interface SubdivisionPageData {
  alpha2: string;
  alpha3: string;
  name: string;
  flag: string;
...
function buildSubdivisionData(alpha2: string): SubdivisionPageData | null {
  ...
  const entries = Object.entries(subs).map(([code, info]) => ({
    code,
    name: info.name,
    type: info.type,
  }));
  ...
  return {
    ...
    currency,
    subdivisions: entries,
    subdivisionTypes: types,
    totalCount: entries.length,
  };
}

const subdivisionDataCache = new Map<string, SubdivisionPageData>();

for (const code of TOP_COUNTRIES) {
  const data = buildSubdivisionData(code);
  if (data) subdivisionDataCache.set(code, data);
}

export function getSubdivisionPageData(alpha2: string): SubdivisionPageData | null {
  return subdivisionDataCache.get(alpha2.toUpperCase()) ?? null;
}

export function getAllSubdivisionCodes(): string[] {
  return [...subdivisionDataCache.keys()];
}
